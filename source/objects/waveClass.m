...
classdef waveClass<handle  
    properties
        ...
        k                           = 0                                    % [rad/m] Wave number (regular waves)
        heading                     = [] % make dependent                  % [deg] Wave heading for regular and irregular waves
        ...
        randPreDefined              = 0;                                   % Only used for irregular waves. Default is equal to 0; if it equals to 1, the waves phase is pre-defined
    end
    
    methods
        
        function obj = waveClass(type,H,T,heading,simu,body,spectrumType,randPreDefined,spectrumDataFile)                % Wave class function
            fprintf('Initializing the Wave Class...\n')
            obj.numFreq = simu.numFreq;
            obj.T = T;
            obj.H = H;
            if isempty(heading)
                obj.heading = body(1).hydro.data.waveheading(1);
            else
                obj.heading = heading;
            end
            obj.type = type;
            ...
            switch type
                ...
                case 'regular'          % Regular Waves
                    ...
                    fprintf('\tWave Heading Beta              (deg) = %G\n',obj.heading)
                ...
                case 'irregularImport'      %  Irregular Waves w/Predefined Phase
                    ...
                    obj.printWaveSpectrumType;
                case 'userdefined'      %  Irregular Waves w/Predefined Phase
                    %obj.printWaveSpectrumType;
                    obj.typeNum = 30;
                    fprintf('\tWave Type                            = User Defined Input Wave Elevation\n')  
                    fprintf('\tWave Heading Beta              (deg) = %G\n',obj.heading)
            end
        end
