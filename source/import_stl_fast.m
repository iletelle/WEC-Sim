...
%
%         Updated by: Michael Lawson (lawsonro3@gmail.com) 10/29/2014
%         fixed mac incompability in matlab 2014b
%-----------------------------------------------------------------
% Ex: 
% filename = 'file.stl'     
...
if ismac == 1 || isunix == 1% what about linux
    fmt = '%s %s %f %f %f \n %s %s \n %s %f %f %f \n %s %f %f %f \n %s %f %f %f \n %s \n %s';
    C=textscan(fid, fmt, 'HeaderLines', 1);
    
    tnorm = cell2mat(C(3:5))
    v1 = cell2mat(C(9:11));
    v2 = cell2mat(C(13:15));
    v3 = cell2mat(C(17:19));
    ...
    if isnan(C{4}(end)) % might need to add this in for the mac version too - check this first if an error occurs
